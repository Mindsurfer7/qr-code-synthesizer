...
bot.command('pay', async (ctx) => {
    try {
        await ctx.replyWithInvoice({
            title: '⭐ Premium Доступ к QR-кодам',
            description: 'Получите безлимитный доступ к созданию QR-кодов премиум-качества',
            payload: `premium_${ctx.from.id}_${Date.now()}`,
            provider_token: '', // Пустая строка для цифровых услуг
            currency: 'XTR',
            prices: [
                { label: 'Premium 1 месяц', amount: 1 }, // 100 Stars
            ],
        });
    }
    catch (error) {
...
bot.on('successful_payment', async (ctx) => {
    try {
        ...
        console.log(`Payment successful: ${paymentId}, amount: ${totalAmount} XTR`);
        // Здесь вы можете добавить логику для предоставления услуги пользователю
        // Например, пометить пользователя как премиум в базе данных
        await ctx.reply('✅ Спасибо за оплату!\n\n' +
            `ID платежа: ${paymentId}\n` +
            'Ваш премиум-доступ активирован!\n\n' +
            'Теперь вы можете использовать бот с полным функционалом.');
    }
    catch (error) {
...
// Обработка команды /terms (требование Telegram)
bot.command('terms', async (ctx) => {
    await ctx.reply('📖 Условия использования\n\n' +
        '1. Бот предназначен для создания QR-кодов для личных и коммерческих целей\n' +
        '2. Мы не несем ответственности за содержимое QR-кодов, созданных пользователями\n' +
        '3. Максимальная длина ссылки: 2048 символов\n' +
        '4. Платежи через Telegram Stars являются окончательными\n' +
        '5. Возврат средств возможен только в случае технических проблем\n\n' +
        'Используя бота, вы соглашаетесь с этими условиями.\n' +
        'Поддержка: /support\n' +
        'Платежная поддержка: /paysupport');
});
...
bot.on((0, filters_1.message)('text'), async (ctx) => {
    ...
    if (state?.awaitingUrl) {
        ...
        try {
            ...
            await ctx.reply('✅ QR-код успешно создан!', telegraf_1.Markup.keyboard([
                ['🔄 Создать QR-код']
            ]).resize());
