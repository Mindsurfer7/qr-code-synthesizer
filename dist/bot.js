...
bot.command('balance', async (ctx) => {
    try {
        ...
        const payments = await database_1.database.getUserPayments(userId);
...
${payments.length > 0 ? '\n📋 Последние 3 покупки:\n' +
            payments.slice(0, 3).map((p, i) => `${i + 1}. ${p.productType === 'high_quality' ? '🖥️ High' : '🎨 Ultra'} - ${p.quantity} шт. (${p.amount}⭐)`).join('\n') : ''}
...
bot.command('pay', async (ctx) => {
    try {
        const user = await database_1.database.getUser(ctx.from.id);
        if (!user) {
            await ctx.reply('Ошибка: пользователь не найден. Используйте /start');
            return;
        }
...
Выберите номер пакета (1-4):
        ...
        await ctx.reply(message, telegraf_1.Markup.inlineKeyboard([
            [
                telegraf_1.Markup.button.callback('1️⃣ Старт (1⭐)', 'buy_high_10'),
                telegraf_1.Markup.button.callback('2️⃣ Профи (1⭐)', 'buy_high_50')
            ],
            [
                telegraf_1.Markup.button.callback('3️⃣ VIP (1⭐)', 'buy_ultra_10'),
                telegraf_1.Markup.button.callback('4️⃣ Premium (1⭐)', 'buy_ultra_50')
            ]
        ]));
    }
    catch (error) {
...
bot.on('successful_payment', async (ctx) => {
    try {
        ...
        if (payloadParts.length >= 3) {
            ...
            await database_1.database.savePayment({
            ...
            if (quality === 'high' || quality === 'ultra') {
                await database_1.database.addPremiumQR(ctx.from.id, quality, quantity);
            }
...
bot.command('paysupport', async (ctx) => {
    await ctx.reply('💳 Поддержка по платежам\n\n' +
...
bot.hears('🖥️ Высокое (800px)', async (ctx) => {
    ...
    if (state?.awaitingQuality) {
        ...
        if (!user || user.premiumHighAvailable <= 0) {
            await ctx.reply(`❌ У вас нет доступных QR-кодов высокого качества (800x800px)\n\n` +
                `Ваш баланс: ${user?.premiumHighAvailable || 0} шт.\n\n` +
                `💳 Чтобы получить доступ, используйте команду /pay\n\n` +
                `📱 Или выберите бесплатное качество (400x400px)`, telegraf_1.Markup.keyboard([
        ...
        state.quality = 'high';
        userStates.set(userId, state);
        await ctx.reply(`✅ Есть доступ! (Осталось: ${user.premiumHighAvailable} шт.)\n\nВыбери тип QR-кода:`, telegraf_1.Markup.keyboard([
            ['✅ С логотипом'],
            ['❌ Без логотипа'],
            ['🔙 Отменить']
        ]).resize());
...
bot.hears('🎨 Ультра (1600px)', async (ctx) => {
    ...
    if (state?.awaitingQuality) {
        ...
        if (!user || user.premiumUltraAvailable <= 0) {
            await ctx.reply(`❌ У вас нет доступных QR-кодов ультра качества (1600x1600px)\n\n` +
                `Ваш баланс: ${user?.premiumUltraAvailable || 0} шт.\n\n` +
                `💳 Чтобы получить доступ, используйте команду /pay\n\n` +
                `📱 Или выберите другое качество`, telegraf_1.Markup.keyboard([
                ['📱 Стандартное (400px)', '🖥️ Высокое (800px)'],
                ['🔙 Отменить']
            ]).resize());
            return;
        ...
        state.quality = 'ultra';
        userStates.set(userId, state);
        await ctx.reply(`✅ Есть доступ! (Осталось: ${user.premiumUltraAvailable} шт.)\n\nВыбери тип QR-кода:`, telegraf_1.Markup.keyboard([
            ['✅ С логотипом'],
            ['❌ Без логотипа'],
            ['🔙 Отменить']
        ]).resize());
...
bot.on((0, filters_1.message)('text'), async (ctx) => {
    ...
    if (state?.awaitingUrl) {
        ...
        try {
            ...
            let remaining = 0;
            if (quality === 'standard') {
                hasAccess = await database_1.database.useFreeStandardQR(userId);
                if (hasAccess) {
                    const user = await database_1.database.getUser(userId);
                    remaining = user?.freeStandardAvailable || 0;
                }
                else {
                    await ctx.reply('❌ У вас закончились бесплатные QR-коды!\n\n' +
                        '💳 Используйте /pay чтобы купить премиум пакеты.');
                    return;
                }
            }
            else if (quality === 'high') {
                hasAccess = await database_1.database.usePremiumHighQR(userId);
                if (hasAccess) {
                    const user = await database_1.database.getUser(userId);
                    remaining = user?.premiumHighAvailable || 0;
                }
                else {
                    await ctx.reply('❌ У вас нет доступных QR-кодов высокого качества!\n\n' +
                        '💳 Используйте /pay чтобы купить пакет.');
                    return;
                }
            }
            else if (quality === 'ultra') {
                hasAccess = await database_1.database.usePremiumUltraQR(userId);
                if (hasAccess) {
                    const user = await database_1.database.getUser(userId);
                    remaining = user?.premiumUltraAvailable || 0;
                }
                else {
                    await ctx.reply('❌ У вас нет доступных QR-кодов ультра качества!\n\n' +
                        '💳 Используйте /pay чтобы купить пакет.');
                    return;
                }
            }
            ...
            await ctx.reply(`✅ QR-код успешно создан (${qualityName})!\n\n` +
                `${qualityEmoji} Осталось: ${remaining} шт.`, telegraf_1.Markup.keyboard([
                ['🔄 Создать QR-код']
            ]).resize());
